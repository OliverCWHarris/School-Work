    ...
    while Again == "y":
        ...
        if len(Filename) > 0:
            ...
            while in_check==False:
                grid_size=int(input('enter grid size: 5-9\n>'))
                if grid_size<5 or grid_size>9:
                    print("not valid size")
                elif grid_size>=5 and grid_size<=9:
                    #MyPuzzle = Puzzle(grid_size, int(8 * 8 * 0.6))
                    MyPuzzle = Puzzle(grid_size, int(5))
                    in_check=True
                else:
                    print('error line 25')
                    quit()
        ...
        with open('high score table.txt','a') as f:
            f.write(username,'\n')
            f.write(Score)
